...
const UI = {
    ...
    updateQuickActions() {
        ...
        quickActionsContainer.innerHTML = ind.quickActions
            .map(
                action =>
    ...
    updateMenuSection() {
        ...
        const isFood = [
            ...
            'sushi',
            'fastfood'
        ].includes(State.currentIndustry);
        ...
        menuGrid.innerHTML = ind.menuItems
            .map(
                item => `
            <div class="menu-item">
    ...
    handleOrderConfirmed() {
        ...
        if (DOM.smsStatus) {
            DOM.smsStatus.textContent = '✓ Sent';
            DOM.smsStatus.classList.add('sent');
        }
        if (DOM.ticketStatus) {
            DOM.ticketStatus.textContent = '✓ Done';
            DOM.ticketStatus.classList.add('sent');
        }

        const orderNum = Math.floor(Math.random() * 900) + 100;

        if (DOM.smsPreview) {
            DOM.smsPreview.innerHTML = Templates.getSMS(
        ...
        if (DOM.ticketPreview) {
            DOM.ticketPreview.innerHTML = Templates.getTicket(
                State.currentIndustry,
                orderNum,
                State.orderTotal,
                '$'
    ...
    resetUI() {
        ...
        if (DOM.smsStatus) {
            DOM.smsStatus.textContent = 'Pending';
            DOM.smsStatus.classList.remove('sent');
        }
        if (DOM.ticketStatus) {
            DOM.ticketStatus.textContent = 'Pending';
